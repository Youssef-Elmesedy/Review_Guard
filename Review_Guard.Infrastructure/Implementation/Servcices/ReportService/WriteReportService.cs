...
internal sealed class WriteReportService : IWriteReportService
{
        ...
        Guid userId, CreateReportRequest request, CancellationToken ct = default)
    {
        try
        {
            ...
            if (existing is not null)
                return Result<ReportResponseDto>.Failure(
                    AppErrorsCataloge.Conflict(_localizer[ReportMessage.AlreadyReported]));
        ...
        {
            return Result<ReportResponseDto>.Failure(AppErrorsCataloge.Failure(_localizer[ex.MessageKey]));
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error creating report for user {UserId}", userId);
            return Result<ReportResponseDto>.Failure(
                AppErrorsCataloge.Failure(_localizer[ReportMessage.CreateFailed]));
        }
    }
        ...
        Guid adminId, Guid reportId, AdminReportActionRequest request, CancellationToken ct = default)
    {
        try
        {
            ...
            if (report is null)
                return Result.Failure(AppErrorsCataloge.NotFound(_localizer[ReportMessage.NotFound]));
        ...
        catch (Exception ex)
        {
            ...
            return Result.Failure(AppErrorsCataloge.Failure(_localizer[ReportMessage.ResolveFailed]));
        }
    }
        ...
        Guid adminId, Guid reportId, AdminReportActionRequest request, CancellationToken ct = default)
    {
        ...
        catch (Exception ex)
        {
            ...
            return Result.Failure(AppErrorsCataloge.Failure(_localizer[ReportMessage.DismissFailed]));
        }
    }
}
